...
# final min-H
parameters1 = np.array([-0.1834604542930385, -0.3496269829727908, 0.18186785252811086, 0.31594719409521466, 0.11153802520443766, -0.017242286625049855, 0.03603432491047065, 0.0977770265727365, 0.05611231969859974, -0.05103240218784337, 
                        -0.25251588658718505, 0.2756148054287644, -0.047245094442072345, -0.08539742086472024, -0.22730634017061285, -0.34979919160066114, -0.3184327024562735, -0.20917234706849588, -0.19841673548299582, 0.24667028868760313, 
                        0.12794501921325233, -0.19787727251549236, -0.16462858651197054, 0.21434220268261545, -0.34893500933818244, -0.10666275966038626, -0.8850979378666923, -1.0071328289727184, 0.779428803450174, -2.4089628269271612, 
                        -1.4188735950218705, 2.2929593015544314, 1.4816895714457101, -2.704204454119533, -2.365605698179186])
...
for mul in np.linspace(0.9, 1.1, num=6, endpoint=True):
    var_parameters = parameters.copy()
    var_parameters[26] *= mul  # 0, 4, 7, 13, 16, 17, 19, 20, 29, 30, 31
    material.update_parameters(*var_parameters)
    fitted_bands, fitted_spins, _ = eigen_solver.solve_BZ_path(get_spin=True)
    fits.append([fitted_bands, fitted_spins])
    real_spins = np.ones_like(fitted_spins)
    
plotting.plot_Ek_output_target_ss([real_bands, real_spins], fits, "fit_thick")
