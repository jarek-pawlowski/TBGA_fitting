...
class Flake:
    ...
    """
    def __init__(self, lattice_constant, side_size, shape='hexagonal', find_neighbours=True):
    ...
    def create_hexagonal_flake(self):
        ...
        """
        self.no_of_nodes *= 3 * 2  # elongated flake
        nodes = np.zeros((self.no_of_nodes,2))
    ...
    def create_rhombus_flake(self):
        ...
        nodetype: ndarray
            with respective type of nodes (1 = M, 2 = X2)
        """
        self.no_of_nodes *= 1  # elongated flake
        nodes = np.zeros((self.no_of_nodes,2))
        nodestype = np.zeros(self.no_of_nodes, dtype=np.int8)
        ...
        for _ in range(self.side_size[0]):
            for _ in range(self.side_size[1]):
                ...
                x = x + self.lattice_constant
                k = k + 1
            y += np.sqrt(3.)/2.*self.lattice_constant 
            x -= self.lattice_constant*(self.side_size[1]+.5)    
        self.no_of_nodes = k
        nodes = nodes[:k,:]
        nodestype = nodestype[:k]
        print("number of flake nodes = " + str(self.no_of_nodes))
...
class Planewaves:
    ...
    def __init__(self, flake, flake_model):
        self.flake = flake
        self.flake_model = flake_model
        self.subspaces = [self.flake.nodes_k]
        self.m = self.flake_model.m
    ...
    def build_plane_hamiltonian(self, kx, ky):
        ...
        # zeeman
        diagonal[:self.m.dim12] += self.flake_model.zeeman(0.,0.) + 1.e-8  # some energy to lift spin degeneracy
        diagonal[self.m.dim12:] -= self.flake_model.zeeman(0.,0.) - 1.e-8
        ...
        # hoppings
        for h in self.flake.hoppingsMM:
            hh_m += self.flake_model.hopping_matrix_6(0., 0., h[0], h[1], 1)*np.exp(1.j*(kx*h[0]+ky*h[1]))
        return hh_m
    ...
    def solve_BZ_path(self):
        return np.array([eigh(self.build_plane_hamiltonian(k[0],k[1]), eigvals_only=True) for k in self.flake.BZ_path])
    ...
    def potential_elements(self, sign=1):
        ...
        unique_nodes = np.array([1], dtype=np.int8)  # np.unique(self.flake.nodestype)
        nodes = [self.flake.nodes[self.flake.nodestype==1]]  # [self.flake.nodes[self.flake.nodestype==unique_node] for unique_node in unique_nodes]
        potential = [self.flake.potential[self.flake.nodestype==1]]  # potential = [self.flake.potential[self.flake.nodestype==unique_node] for unique_node in unique_nodes]
        #
    ...
    def build_hamiltonian(self, include_diagonal=True):
        ...
        pM = self.pot_elements[0]
        #pX2 = self.pot_elements[1]
...
class FlakeMethods:
    ...
    def calculate_densities_k(self, eigenvectors, calculate_spin_valley=False, calculate_real_states=False, every_n=1):
        ...
        for i in range(no_of_eigenstates):
            ...
            S = 0.
            K = 0.
            for [x,y], type in zip(self.flake.nodes[::every_n], self.flake.nodestype[::every_n]):
...
class PlottingOnFlake:
    ...
    def plot_berry_flake(self, field, filename='berry_flake.png'):
        fig, ax = plt.subplots()
        ax.set_aspect(1.)
        ax.set_xlabel("x (nm)")
        ax.set_ylabel("y (nm)")
        fieldonflake = ax.scatter(x=field[:,0]*au.Ah, y=field[:,1]*au.Ah, c=field[:,2], s=self.pointsize, cmap='plasma')
...
class PlottingFourier:
    ...
    def plot_dressed_elements(self, basis_k, dressed_elements, suffix=None):  # dressed_elements is q,k indexed
        ...
        nonzero = np.where(value > 1.e-10)
        value = value[nonzero]
        kx = basis_k[nonzero,0]
        ky = basis_k[nonzero,1]
        elements = ax.scatter(x=kx/au.Ah, y=ky/au.Ah, 
                              c=value, s=self.pointsize, cmap='viridis', norm=colors.LogNorm(vmin=1.e-6, vmax=1.e1))  # norm=colors.LogNorm(vmin=1.e-15)
        cbar = plt.colorbar(elements)
